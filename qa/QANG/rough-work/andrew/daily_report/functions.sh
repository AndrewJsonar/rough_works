...
get_sonar_versions(){
  ...
  if [ -z "${values[sonarw.version]}" ]; then
    values[sonarw.version]="\"N/R\"";
  else
    values[sonarw.version]="\"${values[sonarw.version]}\""
  fi
...
get_kibana_errors(){
  ...
  local log_path=${4};
   ...
   sed -n "$start_index,$ p" "$origin_log_path" | grep -E "warning|error|fatal|exception"| grep -vE "INFO|hits" >> "$log_path";
